...
/**
 * 文件重命名脚本 V8
 * 功能：扫描指定文件夹，将文件名从格式 "『尸鬼』动画原声带 无损音质-P21 21 - 文件名称.mp3"
 *       重命名为 "文件名称.mp3"
 * 
 * 使用方法：
 *   node scripts/rename-files-v8.js <文件夹路径>
 * 
 * 例如：
 *   node scripts/rename-files-v8.js "C:\Users\Username\Music"
 *   node scripts/rename-files-v8.js "F:/RESP/cate_尸鬼"
 */
...
// 例如：『尸鬼』动画原声带 无损音质-P21 21 - 文件名称.mp3
const FILENAME_PATTERN = /-P\d+\s+\d+\s+-\s*(.+?)(\.[^.]+)$/;
...
function scanAndRename(folderPath) {
  if (!fs.existsSync(folderPath)) {
    ...
    console.error('  2. 在 PowerShell 中，建议使用引号包裹路径: node scripts/rename-files-v8.js "F:\\RESP\\cate_尸鬼"');
    console.error('  3. 或者使用正斜杠: node scripts/rename-files-v8.js "F:/RESP/cate_尸鬼"');
    console.error('  4. 或者使用单引号: node scripts/rename-files-v8.js \'F:\\RESP\\cate_尸鬼\'');
    process.exit(1);
  }
...
function main() {
  ...
  if (args.length === 0) {
    console.log('使用方法: node scripts/rename-files-v8.js <文件夹路径>');
    console.log('\n示例:');
    console.log('  node scripts/rename-files-v8.js "C:\\Users\\Username\\Music"');
    console.log('  node scripts/rename-files-v8.js "F:/RESP/cate_尸鬼"');
    console.log('  node scripts/rename-files-v8.js "./music"');
    console.log('\n注意: 在 PowerShell 中，建议使用引号包裹路径，或使用正斜杠');
    console.log('\n支持的文件名格式:');
    console.log('  原格式: "『尸鬼』动画原声带 无损音质-P21 21 - 文件名称.mp3"');
    console.log('  新格式: "文件名称.mp3"');
    process.exit(1);
  }
