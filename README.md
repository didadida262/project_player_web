...
一个面向本地资源的多模态流媒体播放器，结合 Express 后端、Vite + React 前端和 Electron 桌面端，目标是在 macOS 上提供目录浏览、边看边下、视频/音频播放、PDF/图片预览与国际化的完整体验。
...
- **Electron**：主进程在 `electron/main.js` 同步启动 Express 后端，开发模式加载 Vite，生产加载 `frontend/dist`（`base: "./"`）；`electron-builder` 在根 `package.json > build` 控制 `Miles Player` 名称、`dist_electron` 输出、mac `dmg/zip` 以及 icon/appId。
...
- 💽 **打包友好**：`install-deps.sh` 依次安装 backend/frontend/root 依赖，`yarn dev` 并行 Vite + Electron，`yarn build && yarn dist` 生成 mac `.dmg/.zip`。
...
# 打包（生成 dist_electron/Project Player.dmg）
yarn build
yarn dist
```
