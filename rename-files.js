...
/**
 * 文件重命名脚本
 * 功能：扫描指定文件夹，将文件名从格式 "【许嵩】经典歌曲MV合集（收藏） (P1. 文件名).flv"
 *       重命名为 "文件名.flv"
 * 
 * 使用方法：
 *   node rename-files.js <文件夹路径>
 * 
 * 例如：
 *   node rename-files.js "C:\Users\Username\Videos"
 */
...
function scanAndRename(folderPath) {
  if (!fs.existsSync(folderPath)) {
    ...
    console.error('  2. 在 PowerShell 中，建议使用引号包裹路径: node rename-files.js "F:\\RESP\\cate_许嵩"');
    console.error('  3. 或者使用正斜杠: node rename-files.js "F:/RESP/cate_许嵩"');
    console.error('  4. 或者使用单引号: node rename-files.js \'F:\\RESP\\cate_许嵩\'');
    process.exit(1);
  }
...
function main() {
  ...
  if (args.length === 0) {
    console.log('使用方法: node rename-files.js <文件夹路径>');
    console.log('\n示例:');
    console.log('  node rename-files.js "C:\\Users\\Username\\Videos"');
    console.log('  node rename-files.js "F:/RESP/cate_许嵩"');
    console.log('  node rename-files.js "./videos"');
